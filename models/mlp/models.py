...
class CoulombPreprocess(nn.Module):
    def __init__(self, matrix_size, step=1.0, noise=1.0):
        super(CoulombPreprocess, self).__init__()
        self.step = step
        self.noise = noise
    ...
    def realize(self, X):
        ...
        for x in X:
            ...
            inds = torch.argsort(-norms + torch.randn_like(norms) * self.noise)
...
class MLP(nn.Module):
    def __init__(self, matrix_size, output_size, hidden_sizes=[400, 100], activation_type="tanh"):
        ...
        self.preprocess = CoulombPreprocess(matrix_size)
...
class ModelPL(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # Using LambdaLR to adjust learning rate based on epoch
    ...
    def validation_step(self, data, batch_idx):
        ...
        self.log("val_mae", self.val_mae, on_step=False, on_epoch=True, prog_bar=True, batch_size=self.batch_size)
